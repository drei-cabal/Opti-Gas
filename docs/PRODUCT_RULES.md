...
This project keeps the formulas for recommendation filters, cost scoring, ranking, and user-facing recommendation reasons in `utils/product_rules/`.

The goal is to keep product decisions separate from HTTP handling and response shaping. `utils/recommendation_pipeline.py` still owns the full recommendation pipeline, but it calls product-rule modules for the actual calculations.

## Folder Map

| File | Owns |
| --- | --- |
| `utils/product_rules/presets.py` | Recommendation modes, scoring weights, and default trip assumptions |
| `utils/product_rules/filters.py` | Brand and radius filter rules |
| `utils/product_rules/cost.py` | Reference-price selection and expected fuel-spend formulas |
| `utils/product_rules/normalization.py` | Metric normalization for weighted scoring |
| `utils/product_rules/ranking.py` | Final score calculation and deterministic sort order |
| `utils/product_rules/explanations.py` | Primary and secondary recommendation reason rules |
| `utils/product_rules/display.py` | Public display values for distance, duration, and trip cost |

`utils/scoring.py` and `utils/filters.py` remain as compatibility wrappers. New backend code should import directly from `utils/product_rules/`.
...
Change these weights only in `utils/product_rules/presets.py`.
